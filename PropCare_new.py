...
class Causal_Model(Model, ABC):
    def __init__(self, num_users, num_items, flags, user_embs, item_embs, item_popularity, **kwargs):
        ...
        self.ablation_variant = getattr(flags, "ablation_variant", "baseline")
        ...
        # LayerNorm и Dropout
        if self.ablation_variant == "no_layernorm":
            self.layer_norm = lambda x: x
        else:
            self.layer_norm = tf.keras.layers.LayerNormalization()

        if self.ablation_variant == "no_dropout":
            self.relevance_dropout = lambda x, training=None: x
        else:
    ...
    def propensity_train(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            loss_relevance = tf.constant(0.0)
            loss_propensity = tf.constant(0.0)

            if self.ablation_variant != "no_bce_losses":
